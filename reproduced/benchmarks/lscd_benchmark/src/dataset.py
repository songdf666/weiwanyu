...
class Dataset(BaseModel):
    ...
    def stats_groupings_df(self) -> DataFrame:
        if self._stats_groupings is None:
            candidates = [
                self.absolute_path / "stats" / "semeval" / "stats_groupings.csv",
                self.absolute_path / "stats" / "semeval" / "stats_groupings.tsv",
                self.absolute_path / "stats" / "opt" / "stats_groupings.csv",
                self.absolute_path / "stats" / "opt" / "stats_groupings.tsv",
                self.absolute_path / "stats" / "stats_groupings.csv",
                self.absolute_path / "stats" / "stats_groupings.tsv",
            ]
            path = next((candidate for candidate in candidates if candidate.exists()), candidates[-2])
